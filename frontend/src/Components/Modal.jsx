...
import { deleteApi } from "../utils/ApiHandler";
...
export default function Modal({
  ...
  const handleDelete = async () => {
    try {
      const response = await deleteApi(`api/users/${selectedUser.id}`);

      setOpen(false);
      onDeleteSuccess(selectedUser.id);
    } catch (error) {}
  };
  return (
                ...
                <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                  ...
                  <div className="mt-2">
                    <p className="text-sm text-gray-500">
                      ...
                      <strong>{selectedUser?.first_name}</strong>? This action
                      cannot be undone.
                    </p>
                  </div>
                </div>
